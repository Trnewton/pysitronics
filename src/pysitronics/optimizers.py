...
class FORCE(Abstract_Optimizer):
    def __init__(self, alpha, supervisor, dt, warmup_steps, training_steps, learn_step, I_in=None) -> None:
        super().__init__()
...
class EchoState(Abstract_Optimizer):
    def __init__(self, alpha, supervisor, dt, warmup_steps, training_steps, I_in=None) -> None:
        self.alpha = alpha
        self.sup = supervisor
        self.dt = dt
        self.warmup_steps = warmup_steps
    ...
    def teach_network(self, network: nn.Abstract_Network) -> None:
        ...
        dim = self.sup.shape[1] if len(self.sup.shape) > 1 else 1
        RR = np.eye(network.N) / self.alpha
        Rz = np.zeros((network.N, dim))
        _ = network.simulate(self.dt, self.warmup_steps)

        for i, z_i in enumerate(self.sup):
            network.z = z_i
            _, R = network.step(self.dt)
            RR += np.outer(R, R)
            Rz += np.outer(R, z_i)

        phi = np.matmul(np.linalg.inv(RR), Rz)
        network.set_decoder(phi.T)
...
class Basic_Eval(Abstract_Evaluator):
    def __init__(self, supervisor:np.ndarray, eval_steps:int, sample_rate:int,\
                 dt:float, net_size:int, I_in:np.ndarray=None) -> None:
        ''''''
        self.sup = supervisor
        self.I_in = I_in
